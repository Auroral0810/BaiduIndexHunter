...
// 获取配置项类型
// 需要下拉选择的配置项及其选项（使用 computed 确保语言切换时动态更新）
const selectOptions = computed<Record<string, { label: string; value: string }[]>>(() => ({
...
const getConfigType = (key: string, value: any) => {
  ...
  // 检查是否有预定义的下拉选项
  if (selectOptions.value[key]) return 'select'
      ...
      <div class="api-auth-compact">
        <div class="api-status-indicator">
          <el-tag 
            ...
            round
          >
            <el-icon class="status-icon"><component :is="apiSecretKey ? Connection : Lock" /></el-icon>
            {{ apiSecretKey ? $t('views.settings.api_configured') : $t('views.settings.api_not_configured') }}
          </el-tag>
        </div>
        
        <div class="api-input-compact">
          <el-input
            ...
            clearable
            class="compact-api-input"
            @blur="saveApiKey"
          >
          </el-input>
        </div>
        
        <el-tooltip :content="$t('views.settings.api_tip')" placement="bottom" effect="light">
          <el-icon class="api-help-icon"><InfoFilled /></el-icon>
        </el-tooltip>
      </div>
    </header>
...
.compact-api-input :deep(.el-input__wrapper) {
...
.compact-api-input :deep(.el-input__wrapper.is-focus) {
