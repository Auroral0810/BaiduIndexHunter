<template>
  <div class="region-city-selector">
    ...
    <div class="selector-container">
      ...
      <div class="province-list">
        ...
        <div class="province-item special-item" @click="selectNationwide">
          <el-checkbox
            ...
            @click.stop
            >{{
              $t("components-RegionCitySelector-19c298e1bc4e14bf9-1")
            }}</el-checkbox
          >
        </div>
        <!-- 省份选择操作 -->
      ...
      <div class="city-list">
          ...
          <div class="city-header">
            ...
            <div class="actions">
              <el-button
                ...
                >{{
                  $t("components-RegionCitySelector-19c298e1bc4e14bf9-6")
                }}</el-button
              >
              <el-button
                type="text"
                ...
                >{{
                  $t("components-RegionCitySelector-19c298e1bc4e14bf9-7")
                }}</el-button
              >
            </div>
          </div>
        ...
        <div v-else class="no-city-selected">
          {{ $t("components-RegionCitySelector-19c298e1bc4e14bf9-8") }}
        </div>
      </div>
    </div>
    <!-- 已选城市展示区域 -->
    <div
      ...
      v-if="hasSelectedCities || nationwideSelected || hasSelectedProvinces"
    >
      ...
      <div class="selected-tags">
        ...
        <el-tag
          ...
          class="city-tag"
          >{{
            $t("components-RegionCitySelector-19c298e1bc4e14bf9-11")
          }}</el-tag
        >
        <!-- 省份标签 -->
        <el-tag
        ...
        >
          + {{ remainingCityCount }}
          {{ $t("components-RegionCitySelector-19c298e1bc4e14bf9-12") }}</el-tag
        >
        <!-- 收起按钮 -->
        <el-button
          v-if="
            showAllCities && selectedCityCodesList.length > CITY_DISPLAY_LIMIT
          "
          type="primary"
          link
          size="small"
          ...
          class="collapse-btn"
          >{{
            $t("components-RegionCitySelector-19c298e1bc4e14bf9-13")
          }}</el-button
        >
      </div>
    </div>
...
const currentProvinceCities = computed(() => {
  if (
    ...
    !provinceCities.value[selectedProvince.value]
  ) {
    return null;
  }
...
const hasSelectedProvinces = computed(() => {
  return Object.values(selectedProvinceDirectly).some(
...
const selectedProvincesList = computed(() => {
  return Object.keys(selectedProvinceDirectly).filter(
    (code) => selectedProvinceDirectly[code],
  );
});
...
// 监听选中状态变化，更新v-model
watch(
  [cityChecked, selectedProvinceDirectly, nationwideSelected],
  () => {
...
onMounted(async () => {
  ...
  try {
    ...
    console.error(
      t("components-RegionCitySelector-19c298e1bc4e14bf9-14"),
      err,
    );
    error.value = t(
