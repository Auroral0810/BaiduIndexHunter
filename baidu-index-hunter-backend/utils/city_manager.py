...
# 默认城市代码文件路径
DEFAULT_CITY_FILE = Path(__file__).parent.parent / 'data' / '275个城市及代码.xlsx'

class CityManager:
    """城市代码管理器，提供城市代码和名称的映射关系"""
    
    def __init__(self, city_file=None):
        """
        初始化城市管理器
        :param city_file: 城市代码文件路径，默认使用内置文件
        """
        self.city_file = city_file or DEFAULT_CITY_FILE
        self.city_dict = {}  # 城市代码到名称的映射
        self.city_name_dict = {}  # 城市名称到代码的映射
        self.load_city_data()
    ...
    def load_city_data(self):
        ...
        try:
            if os.path.exists(self.city_file):
                # 尝试从Excel文件加载
                df = pd.read_excel(self.city_file)
                for _, row in df.iterrows():
                    # 假设Excel文件有"城市代码"和"城市名称"两列
                    if '城市代码' in df.columns and '城市名称' in df.columns:
                        code = int(row['城市代码'])
                        name = row['城市名称']
                        self.city_dict[code] = name
                        self.city_name_dict[name] = code
                    else:
                        # 如果列名不匹配，尝试使用前两列
                        code = int(row.iloc[0])
                        name = row.iloc[1]
                ...
                log.info(f"从文件加载了 {len(self.city_dict)} 个城市代码")
            else:
                # 如果文件不存在，使用硬编码的城市数据
                self._load_hardcoded_city_data()
                log.warning(f"城市代码文件不存在: {self.city_file}，使用硬编码数据")
        except Exception as e:
            log.error(f"加载城市数据失败: {e}")
            # 使用硬编码的城市数据作为备份
            self._load_hardcoded_city_data()
    
    def _load_hardcoded_city_data(self):
        """加载硬编码的城市数据"""
        # 这里包含了常用的城市代码，完整列表可以从您提供的代码中提取
        self.city_dict = {
        ...
        log.info(f"加载了 {len(self.city_dict)} 个硬编码城市代码")
    ...
    def get_city_name(self, city_code):
        ...
        """
        return self.city_dict.get(int(city_code) if isinstance(city_code, (int, str)) else city_code)
    ...
    def get_all_city_names(self):
        ...
        :return: 城市名称列表
        """
        return list(self.city_name_dict.keys())
    
    def get_all_city_codes(self):
        """
        获取所有城市代码
        :return: 城市代码列表
        """
