...
# 2. 资讯指数（包括日度数据、年度数据以及周度数据，以及时间区间内的日均值、同比、环比）可选的条件参数：时间：7天、30天、90天、半年，以及自定义时间；城市：全国，375个地级市；
# 请求示例和参数如下：
keyword = '电脑'
area = 0
...
url = f"{BAIDU_INDEX_API['trend_url']}?area={area}&word=[[{{\"name\":\"{encoded_keyword}\",\"wordType\":1}}]]&startDate={start_date}&endDate={end_date}"
# url = f"{BAIDU_INDEX_API['trend_url']}?area={area}&word=[[{{\"name\":\"{encoded_keyword}\",\"wordType\":1}}]]&days={days}"
# days可以=7、30、90、180或者不加days参数，就代表全部。area默认为0，代表全国
# word可以为：[[{"name":"电脑","wordType":1}],[{"name":"衣服","wordType":1}]]这样多个参数，表示对比
# 生成Cipher-Text参数
# words={encoded_keyword}里面的encoded_keyword是一个用逗号分开的字符串，比如words=%E7%94%B5%E8%84%91,%E8%A1%A3%E6%9C%8D，而前面的encoded_keyword则是第一个参数，即电脑的话就是%E7%94%B5%E8%84%91
word1 = f'{encoded_keyword},手机'
cipher_url = f"{BAIDU_INDEX_API['referer']}#/trend/{encoded_keyword}?words={word1}"
